...
- 결과: 포그라운드면 토스트("스크린샷을 정리하고 있어요" → "정리했어요"), 몇 초 내 홈 목록 갱신
...
### 동작 방식(요약)
- 포그라운드: 감지 → 업로드 → 온디바이스 OCR → GPT 정리 → 저장 → 토스트 + 목록 갱신
- **앱 시작 직후(JS 로딩 중) 찍은 스크린샷**: 로딩 완료 시 자동 회수(checkNow) ✓ 검증됨
- **백그라운드 중 찍은 스크린샷**: 복귀 시 자동 회수 ✓ (iOS 검증됨). 파이프라인이 백그라운드에서
  완료되면 시스템 알림으로 결과 통지(expo-notifications 로컬 알림)
